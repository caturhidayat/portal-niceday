                ...
                <div className="grid gap-2">
                    <form className="space-y-2" onSubmit={handleSubmit}>
                        ...
                        <div className="grid gap-4">
                            ...
                            <div className="flex items-center gap-2">
                                <Label htmlFor="description" className="w-60">Description</Label>
                                <Input id="description" type="text" name="description" placeholder="Enter Dedcription" className="flex-grow" />
                            </div>
                            <div className="flex items-center gap-2">
                                    ...
                                    <PopoverContent>
                                        <Calendar mode="single" selected={date} onSelect={(newDate) => {
                                            ...
                                            if (newDate) {
                                                ...
                                                const hiddenInput = document.getElementById("hiddenStartDate") as HTMLInputElement;
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <Input id="cycleLength" name="cycleLength" type="number" value={cycleLength} onChange={(e) => setCycleLength(Number(e.target.value))} className="flex-grow" />
                            </div>
                            <Button type="button" onClick={() => setDisplayShift(!displayShift)}>Add Shift</Button>
                        </div>
                        <div>
                                    ...
                                    <div className="flex items-center gap-2">
                                        ...
                                        <Popover open={open[index]} onOpenChange={(isOpen) => setOpen(prev => ({ ...prev, [index]: isOpen }))}>
                                            <PopoverTrigger asChild>
                                                ...
                                                >
                                                    {value[index]
                                                        ? shifts.find((shift) => shift.name === value[index])?.name
                                                        : "Select shift..."}
                                                ...
                                                <Command>
                                                    ...
                                                    <CommandList>
                                                        ...
                                                        <CommandGroup>
                                                            {shifts.map((shift) => (
                                                                <CommandItem
                                                                    key={shift.id}  // Changed from shift.name to shift.id
                                                                    value={shift.id}  // Changed from shift.name to shift.id
                                                                    onSelect={(currentValue) => {
                                                                        ...
                                                                        const hiddenInput = document.getElementById(`hiddenDay${index}`) as HTMLInputElement;
                                                                        if (hiddenInput) hiddenInput.value = currentValue;  // This now sets the shift id
                                                                    }}
                                                                >
                                                                    <Check
                                                                        className={cn(
                                                                            "mr-2 h-4 w-4",
                                                                            value[index] === shift.id ? "opacity-100" : "opacity-0"  // Changed from shift.name to shift.id
                                                                        )}
                                                                    />
                                                                    {shift.name}
                                                                </CommandItem>
                                        ...
                                        <input type="hidden" id={`hiddenDay${index}`} name={`day-${index + 1}`} />
                                    </div>
                                </div>
                            ))}
                        </div>
